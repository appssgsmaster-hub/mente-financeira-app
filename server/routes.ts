...
export async function registerRoutes(
  ...
  // ── Gestão de Ideias ──────────────────────────────────────────────────────
  app.get("/api/idea-projects", requireActiveSubscription, async (req, res) => {
    const items = await storage.getIdeaProjects(req.session.userId!);
    res.json(items);
  });

  app.post("/api/idea-projects", requireActiveSubscription, async (req, res) => {
    try {
      const { name, description, color, seedStages } = req.body as {
        name?: string; description?: string; color?: string; seedStages?: string[];
      };
      if (!name?.trim()) return res.status(400).json({ message: "Nome é obrigatório" });
      const project = await storage.createIdeaProject({
        userId: req.session.userId!,
        name: name.trim(),
        description: description?.trim() ?? null,
        color: color ?? "violet",
      });
      const defaultStages = seedStages?.length
        ? seedStages
        : ["Diagnóstico", "Planejamento", "Execução", "Monitoramento", "Otimização"];
  ...
  app.patch("/api/idea-projects/:id", requireActiveSubscription, async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      const updated = await storage.updateIdeaProject(req.session.userId!, id, req.body);
      res.json(updated);
    } catch (err) {
      res.status(404).json({ message: (err as Error).message });
    }
  ...
  app.delete("/api/idea-projects/:id", requireActiveSubscription, async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      await storage.deleteIdeaProject(req.session.userId!, id);
      res.json({ ok: true });
    } catch (err) {
      res.status(404).json({ message: (err as Error).message });
    }
  ...
  app.get("/api/idea-projects/:id/stages", requireActiveSubscription, async (req, res) => {
    const projectId = parseInt(req.params.id as string);
    const stages = await storage.getProjectStages(req.session.userId!, projectId);
    res.json(stages);
  });

  app.post("/api/idea-projects/:id/stages", requireActiveSubscription, async (req, res) => {
    try {
      const projectId = parseInt(req.params.id as string);
      const { name } = req.body as { name?: string };
      if (!name?.trim()) return res.status(400).json({ message: "Nome da etapa é obrigatório" });
      const existing = await storage.getProjectStages(req.session.userId!, projectId);
  ...
  app.post("/api/idea-projects/:id/stages/reorder", requireActiveSubscription, async (req, res) => {
    try {
      const projectId = parseInt(req.params.id as string);
      const { orderedIds } = req.body as { orderedIds?: number[] };
      if (!Array.isArray(orderedIds)) return res.status(400).json({ message: "orderedIds deve ser um array" });
      await storage.reorderProjectStages(req.session.userId!, projectId, orderedIds);
      res.json({ ok: true });
    } catch (err) {
      res.status(400).json({ message: (err as Error).message });
    }
  ...
  app.patch("/api/project-stages/:id", requireActiveSubscription, async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      const updated = await storage.updateProjectStage(req.session.userId!, id, req.body);
      res.json(updated);
    } catch (err) {
      res.status(404).json({ message: (err as Error).message });
    }
  ...
  app.delete("/api/project-stages/:id", requireActiveSubscription, async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      await storage.deleteProjectStage(req.session.userId!, id);
      res.json({ ok: true });
    } catch (err) {
      res.status(404).json({ message: (err as Error).message });
    }
