...
export async function registerRoutes(
  ...
  // ── Company Profile ────────────────────────────────────────────────────────
  app.get("/api/company-profile", requireActiveSubscription, async (req, res) => {
    try {
  ...
  app.put("/api/company-profile", requireActiveSubscription, async (req, res) => {
    try {
      const profile = await storage.upsertCompanyProfile(req.session.userId!, req.body);
      res.json(profile);
    } catch (err) {
      res.status(500).json({ message: "Erro ao salvar perfil da empresa" });
    }
  });

  // ── Bank Details ───────────────────────────────────────────────────────────
  app.get("/api/bank-details", requireActiveSubscription, async (req, res) => {
    try {
  ...
  app.put("/api/bank-details", requireActiveSubscription, async (req, res) => {
    try {
      const details = await storage.upsertBankDetails(req.session.userId!, req.body);
      res.json(details);
    } catch (err) {
  ...
  // ── Invoices ───────────────────────────────────────────────────────────────
  app.get("/api/invoices", requireActiveSubscription, async (req, res) => {
    try {
      const list = await storage.getInvoices(req.session.userId!);
      res.json(list);
    } catch (err) {
  ...
  app.get("/api/invoices/:id", requireActiveSubscription, async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      const inv = await storage.getInvoiceWithItems(req.session.userId!, id);
  ...
  app.post("/api/invoices", requireActiveSubscription, async (req, res) => {
    try {
      const { items, ...invoiceData } = req.body;
      const inv = await storage.createInvoice(req.session.userId!, invoiceData, items ?? []);
  ...
  app.put("/api/invoices/:id", requireActiveSubscription, async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      const { items, ...invoiceData } = req.body;
      const inv = await storage.updateInvoice(req.session.userId!, id, invoiceData, items);
      res.json(inv);
    } catch (err) {
  ...
  app.patch("/api/invoices/:id/status", requireActiveSubscription, async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      const { status } = req.body;
      const inv = await storage.updateInvoice(req.session.userId!, id, { status });
      res.json(inv);
    } catch (err) {
  ...
  app.delete("/api/invoices/:id", requireActiveSubscription, async (req, res) => {
    try {
      const id = parseInt(req.params.id as string);
      await storage.deleteInvoice(req.session.userId!, id);
      res.json({ ok: true });
    } catch (err) {
      res.status(500).json({ message: "Erro ao apagar fatura" });
    }
  });
