...
export function useUser() {
  return useQuery({
    ...
    queryFn: () => fetchWithZod<User>(api.user.get.path, {}, api.user.get.responses[200]),
  });
}
...
export function useAccounts() {
  return useQuery({
    ...
    queryFn: () => fetchWithZod<Account[]>(api.accounts.list.path, {}, api.accounts.list.responses[200]),
  });
}
...
export function useTransactions() {
  return useQuery({
    ...
    queryFn: () => fetchWithZod<Transaction[]>(api.transactions.list.path, {}, api.transactions.list.responses[200]),
  });
}
...
export function useUpdateAccountPercentages() {
  ...
  return useMutation({
    mutationFn: async (data: { updates: { id: number; percentage: number }[] }) => {
...
export function useDistributeIncome() {
  ...
  return useMutation({
    mutationFn: async (data: { amount: number; description: string }) => {
      return fetchWithZod<{ transaction: Transaction; updatedAccounts: Account[] }>(
